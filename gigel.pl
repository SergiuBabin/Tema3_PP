...
find_rules(Tokens, Rules) :- rules([Z], Rules), member(Z, Tokens).

% select_answer/5
% select_answer(+Tokens, +UserMemory, +BotMemory, -Answer, -Actions)
select_answer(Tokens, _UserMemory, BotMemory, Answer, Actions) :- 
								find_rules(Tokens, X),  find_matching_rules(Tokens, X, _, Y), 
								find_answers(Y, A, Actions), find_answer(A, BotMemory, K), min_element(K, P),
								words(P, Answer),!.
...
find_answer([], _,[]) :- !.
find_answer([Ans | T], Mem, [(B, K)|A]) :- unwords(Ans, B), get_value(Mem, B, K), find_answer(T, Mem, A).
...
find_answers([], _, _).
find_answers([rule(_, Rule, A, _, _) | T], Answers, A) :-  extract_answer(Rule ,Answers), find_answers(T, Answers, A).

extract_answer([A],[A]) :- !.
extract_answer([A | Rule], [A | X]) :- extract_answer(Rule, X).
